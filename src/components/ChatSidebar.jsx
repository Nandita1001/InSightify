...
export default function ChatSidebar() {
  const { role, roleChats, activeChatId, handleNewChat, switchChat } = useApp();
  ...
  return (
    <div className="w-56 flex-shrink-0 border-r flex flex-col" style={{ background: "#fff", borderColor: "#e8eaed" }}>
      ...
      <div className="px-4 py-3 border-t" style={{ borderColor: "#e8eaed" }}>
        ...
        <p className="text-[11px]" style={{ color: "#9ca3af" }}>{roleChats.length} chat{roleChats.length !== 1 ? "s" : ""}</p>
      </div>
    </div>
  );
}
